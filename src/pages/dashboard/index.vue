        ...
        <template v-for="team in teams" :key="team.id">
          <div class="bg-white rounded-xl shadow-sm cursor-pointer p-5 w-80 flex flex-col justify-between border border-gray-200 hover:shadow-md transition-shadow duration-300">
            <div class="flex items-center justify-between mb-2">
              <span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-sm font-medium mr-2">{{ team.tag }}</span>
              <el-dropdown @command="handleCommand">
                <Icon icon="mdi:dots-vertical" class="cursor-pointer w-5 h-5" />
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item command="settings">管理团队</el-dropdown-item>
            ...
            <div class="text-gray-500 text-sm mb-3">{{ team.desc }}</div>
            <div class="flex justify-between text-gray-400 text-xs">
              <span class="flex items-center"><Icon icon="mdi:user" class="mr-1 w-4 h-4" />{{ team.members }} 成员</span>
