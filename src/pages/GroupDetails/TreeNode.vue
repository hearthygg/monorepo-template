<template>
  <div>
    <div :class="['flex items-center py-1 px-2 rounded cursor-pointer hover:bg-gray-100 transition-colors', isSelected ? 'bg-blue-50 text-blue-600' : 'text-gray-700']" :style="{ paddingLeft: `${level * 16 + 8}px` }" @click="handleClick">
      <button v-if="node.type === 'folder' && hasChildren" class="mr-1 p-0.5 hover:bg-gray-200 rounded" @click.stop="$emit('toggle', node.id)">
        <ChevronDown v-if="isExpanded" class="h-3 w-3" />
        <ChevronRight v-else class="h-3 w-3" />
      </button>

      <FolderOpen v-if="node.type === 'folder' && isExpanded" class="h-4 w-4 mr-2 text-blue-500" />
      <Folder v-else-if="node.type === 'folder'" class="h-4 w-4 mr-2 text-blue-500" />
